/** @file MaxRectsBinPack.cpp
    ...
    Toby Harris added canRotateRects Nov'2013
*/
#include <utility>
#include <iostream>
#include <limits>
...
MaxRectsBinPack::MaxRectsBinPack(int width, int height, bool canRotateRects)
{
	Init(width, height, canRotateRects);
}

void MaxRectsBinPack::Init(int width, int height, bool canRotateRects)
{
	binWidth = width;
	binHeight = height;
    
    canRotate = canRotateRects;
...
Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX) const
{
	...
	for(size_t i = 0; i < freeRectangles.size(); ++i)
	{
		...
		if (canRotate && freeRectangles[i].width >= height && freeRectangles[i].height >= width)
		{
